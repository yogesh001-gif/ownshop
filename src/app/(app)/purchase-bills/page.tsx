...
export default async function PurchaseBillsPage() {
  ...
  const suppliers = await prisma.supplier.findMany({
    ...
    include: { purchases: { select: { id: true, invoiceImageUrl: true } } },
    orderBy: { name: 'asc' },
  });
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100">
          ...
          <p className="text-sm text-gray-500 mt-1">View uploaded invoice photos for each supplier.</p>
        </div>
        <div className="divide-y divide-gray-100">
          {suppliers.length === 0 ? (
            <div className="p-8 text-center text-gray-500">No suppliers found.</div>
          ) : suppliers.map((supplier) => {
            const billsWithImages = supplier.purchases.filter((purchase) => purchase.invoiceImageUrl).length;
            return (
              <Link key={supplier.id} href={`/purchase-bills/${supplier.id}`} className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors group">
